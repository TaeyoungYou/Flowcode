...
flowcode_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern flowcode_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern flowcode_int line; /* source code line number - defined in scanner.c */
...
extern flowcode_void startParser(flowcode_void);
extern flowcode_int startScanner(BufferPointer sc_buf);
...
flowcode_void printParserError(flowcode_string fmt, ...) {
...
flowcode_long getParserFileSize(flowcode_string fname) {
	FILE* input;
...
flowcode_void displayParser(BufferPointer ptrBuffer) {
...
flowcode_void callGarbageCollector(flowcode_void) {
